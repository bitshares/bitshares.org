  ...
  <div canvas="container">
    ...
    <main class="main-content">
      ...
      <section class="section section-x">
        <div class="container">
          <p>Named accounts enable users to easily remember and communicate their account information. We don't use IP addresses to browse the internet or numbers to identify
            our email, so why shouldn't we have human-friendly account names for our blockchain transactions?</p>
<p>Every BitShares blockchain account is assigned a globally unique name that can be selected by its creator. 
There are many uses for named accounts beyond security and simply being an alias to a set of 
<a href="../technology/dynamic-account-permissions" target="_blank">dynamic account permissions</a>.
They can be used as user logins or even mapped to your domain name. Named accounts are also transferable, which means a name can become valuable in its own right.</p>
<p>BitShares blockchain defines a simple algorithm to determine the fee it charges to reserve a new account name.
Names that contain a number, are longer than 8 characters, or contain no vowels are essentially free. Otherwise the
name is priced according to its length. BitShares has the power to adapt to market demands by allowing <a href="../technology/delegated-proof-of-stake-consensus" target="_blank">Delegates</a> to propose a different fee for each length.</p>
<br />
<h2 id="transferring-names">Transferring Names</h2>
<p>A BitShares account can be transferred by updating the permissions used to control it. However, the semantics of
...
future.</p>
<p>When a BitShares user transfers an account name to another user, they use a special transaction that clears all of the links in the
web-of-trust. Both the buyer and the seller are protected by this fact, because simply updating the key that controls
a named account does not signify a legal change in ownership.</p>
      </section>
    </main>
    <!-- END Main container -->
